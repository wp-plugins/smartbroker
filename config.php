...
function sb_plugin_admin_init(){
	...
	add_settings_section('sb_server_settings', 'SmartBroker server settings', 'sb_blank_intro', 'sb_plugin');
	...
	add_settings_section('sb_page_settings', 'Page settings', 'sb_page_intro', 'sb_plugin');
		add_settings_field('sb_search_page', 'SmartBroker search page ID', 'sb_search_page_string', 'sb_plugin', 'sb_page_settings');
...
function sb_page_intro() {
	echo "<p>SmartBroker creates two pages on your WordPress installation, which it uses to find and display listing information:</p><ol><li>A search page</li><li>The listing page (where listings details are displayed)</li></ol>
...
function sb_server_address_string() {
	...
	<p>Your SmartBroker server is the where all your listing data is held. It's also where you go to add, edit and delete your listings.</p>
	<p>Will default to <em>http://demo.smart-broker.co.uk</em> if left blank.</p>";
	}
...
function sb_currency_1_string() {
	...
	To see available currencies, log in to your SmartBroker system and go to 'Taxonomy' > 'Currencies' - any live currency can be used.<br/><br/>
...
function sb_plugin_options() {
	...
	echo "<h3>SmartBroker is a web-based sales listing tool. It can be used to sell anything from boats to houses.</h3>
	<p>To use this SmartBroker plugin with real data from real listings, you'll need an account with us - sign up at
	<a href='http://www.smart-broker.co.uk'>www.smart-broker.co.uk</a>.</p>
	<p>If this is your first SmartBroker installation, we strongly recommend you read the <a href='http://www.smart-broker.co.uk/?page_id=300' target='_blank'>Installation Guide</a>.";
