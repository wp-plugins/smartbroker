...
include_once('listing_clean.php');
include_once('listing_v1.php');
include_once('search_clean.php');
...
//$sb_config['video_link'] = '<a href="http://www.youtube-nocookie.com/embed/%s?rel=0&wmode=opaque&modestbranding=1&showinfo=0&theme=light" rel="sb_prettyPhoto[exterior]">
//<iframe class="video" width="200" height="115"
//src="http://www.youtube-nocookie.com/embed/%s?rel=0&wmode=opaque&modestbranding=1&showinfo=0&theme=light"
//frameborder="0" allowfullscreen></iframe>
//</a>';
...
function sb_listing_func($atts){
	extract(shortcode_atts(array('layout' => 'original', $atts ),$atts));
	if ($layout == 'clean') {
		return sb_listing_clean_func();
		} else {
		return sb_listing_v1_func();
		}
	}
...
function sb_search_by_ref_func($atts) {
	global $sb_config;
	extract(shortcode_atts( array(
		'layout' => 'v1'), $atts ) );
	$a = "<div class='sb_wrapper' style='max-width: 400px;'>
	...
	<div class='ui-widget ui-widget-content ui-corner-bottom content' style='padding: 0em .5em;'>
		...
		<td style='vertical-align: middle;'><button class='button' type='submit'>".__('Go','smartbroker')."</button></td>
		</tr></table>
		</form>
	</div>
	</div>";
	...
	<form method='get' action='".site_url('/')."' target='_parent'>
	<input type='hidden' name='page_id' value='".$sb_config['listing_page']."'/>
	<table><tr><td style='vertical-align: middle;'>Boat reference:</td>
	<td style='vertical-align: middle;'><input type='text' name='boat_id' size='10' value='' /></td>
	<td style='vertical-align: middle;'><input type='submit' value='".__('Go','smartbroker')."' /></td>
	</tr></table>
	</form>";
	if ($layout == 'clean') {
		return $a_clean;
